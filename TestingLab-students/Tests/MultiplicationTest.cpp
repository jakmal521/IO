...
class MultiplicationByIntTests : public ::testing::TestWithParam<tuple<Matrix, double, Matrix>> {

};
TEST_P(MultiplicationByIntTests, CheckMultiplication)
{
	Matrix A = get<0>(GetParam());
	double multiplier = get<1>(GetParam());
...
INSTANTIATE_TEST_CASE_P(MultiplicationByIntTest, MultiplicationByIntTests,
	::testing::Values(
		...
		make_tuple(Matrix(2, 1.0), 0.5, Matrix(2, 0.5)),
		make_tuple(Matrix(2, 2.0), 2, Matrix(2, 4.0)),
		make_tuple(Matrix(1, 2.0), 2, Matrix(1, 4.0)),
